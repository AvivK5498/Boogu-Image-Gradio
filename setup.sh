...
# Boogu's custom transformer (loaded via trust_remote_code) uses flash-attn when available.
# Use the project's own installer, which finds a prebuilt wheel for the GPU (Hopper sm_90 here),
# then falls back to a source build. If it can't, the model still runs (SDPA path) — just slower.
echo "==> flash-attn (Hopper wheel via Boogu's installer; SDPA fallback if unavailable)"
if python -c "import flash_attn" 2>/dev/null; then
  echo "    flash-attn already present."
else
  if [[ ! -d "$BOOGU_SRC" ]]; then
    GIT_LFS_SKIP_SMUDGE=1 git clone --depth 1 https://github.com/boogu-project/Boogu-Image.git "$BOOGU_SRC" || true
  fi
  if [[ -f "$BOOGU_SRC/utils/get_flash_attn.py" ]]; then
